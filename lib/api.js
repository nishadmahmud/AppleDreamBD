...
export const getBanners = () => {
  const endpoint = process.env.NEXT_PUBLIC_ENDPOINT_BANNERS || "/get-banners";
  if (!BLOG_BASE || !STORE_ID) {
    console.error("Missing environment variables: BLOG_BASE or STORE_ID");
  }
  return `${BLOG_BASE}${endpoint}/${STORE_ID}`;
...
export const getCategories = () => {
  const endpoint = process.env.NEXT_PUBLIC_ENDPOINT_CATEGORIES || "/categories";
  if (!API_BASE || !STORE_ID) {
    console.error("Missing environment variables: API_BASE or STORE_ID");
  }
  return `${API_BASE}${endpoint}/${STORE_ID}`;
};
