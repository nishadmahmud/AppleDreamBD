    ...
    <section className="px-4 sm:px-8 lg:px-10 py-10">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-6">
        {[
          ...
          <div
            key={p.title}
            className="flex flex-col gap-3 pb-3 group text-center"
          >
            <div className="w-full bg-gray-100 dark:bg-background-dark/50 rounded-lg overflow-hidden transition-colors duration-300">
              <Image
                ...
                alt={p.title}
                src={p.img}
                className="w-full h-full object-contain p-4 group-hover:scale-105 transition-transform duration-300 aspect-square"
                width={1000}
                height={1000}
              />
            </div>
            <div>
              <p className="text-base font-medium leading-normal text-gray-900 dark:text-white transition-colors duration-300">
                {p.title}
              </p>
              <p className="text-sm font-normal leading-normal text-gray-600 dark:text-gray-400 transition-colors duration-300">
                {p.price}
              </p>
              <a
                href="#"
                className="text-sm font-bold leading-normal text-primary hover:underline"
              >
                Add to Cart
              </a>
            </div>
